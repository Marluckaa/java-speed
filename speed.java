...
public class speed extends JFrame implements ActionListener, KeyListener
{
	...
	public void draw(Graphics g)
	{
		...
		if(running)
		{
			...
			if(isDistyped) {
				isDistyped = false;
				g.setColor(Color.green);
				System.out.println("dis "+disTyped +" typed " + typed);
				
					//g.setColor(Color.green);
					//g.drawString(passage.substring(0,typed), g.getFont().getSize(),g.getFont().getSize()*5);
					g.setColor(Color.red);
					g.drawString(passage.substring(typed,typed+1), g.getFont().getSize(),g.getFont().getSize()*5);
				}
	...
	private double calculateAccuracy(String passage, String typedPass) {
	        int correct = 0;
	        for (int i = 0; i < passage.length(); i++) {
	            if (i < typedPass.length() && typedPass.charAt(i) == passage.charAt(i)) {
	                correct++;
	            }
	        }
	        return (double) correct / passage.length() * 100;
	    }
